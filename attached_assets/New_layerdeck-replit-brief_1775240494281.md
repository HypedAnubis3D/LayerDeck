...
PI_URL                = https://layerdeck-hub.tail5636e6.ts.net/hub
...
```
https://layerdeck-hub.tail5636e6.ts.net/camera/stream.html?src=camera_a1
https://layerdeck-hub.tail5636e6.ts.net/camera/stream.html?src=camera_p1_room
https://layerdeck-hub.tail5636e6.ts.net/camera/stream.html?src=camera_p1_closet
```
...
```javascript
// ── Config ───────────────────────────────────────────
const PI_URL        = 'https://layerdeck-hub.tail5636e6.ts.net/hub';
const GO2RTC_URL    = 'https://layerdeck-hub.tail5636e6.ts.net/camera';
const PRINTER_NAMES = ['A1', 'P1 Room', 'P1 Closet'];
const CAMERA_STREAMS = {
  'A1':        `${GO2RTC_URL}/stream.html?src=camera_a1`,
  'P1 Room':   `${GO2RTC_URL}/stream.html?src=camera_p1_room`,
  'P1 Closet': `${GO2RTC_URL}/stream.html?src=camera_p1_closet`
};
// ─────────────────────────────────────────────────────
...
async function loadHubConfig() {
  ...
  config.printers.forEach(p => {
    CAMERA_STREAMS[p.name] = `${GO2RTC_URL}/stream.html?src=${p.cameraStream}`;
  });
...
```
https://layerdeck-hub.tail5636e6.ts.net/hub/companion
```
...
### Missing Channel Alert Specs — #bambu-restock, #convention-prep, #print-watch

These 3 channels have webhooks configured but their alert logic needs to be wired in. Add the following to the Pi hub server.js and LayerDeck server as noted:

**#bambu-restock** — fires when the Bambu restock monitor (Section 26) detects a filament back in stock.
```javascript
// In restock-monitor.js — call when a monitored item comes back in stock
await postToDiscord(
  process.env.DISCORD_WEBHOOK_BAMBU_RESTOCK,
  `🎉 **Back in stock:** ${itemName}
${itemUrl}`
);
```

**#convention-prep** — fires at 7, 3, and 1 day before each convention event with a low-stock checklist.
```javascript
// In convention-prep.js — fires on schedule for upcoming events
await postToDiscord(
  process.env.DISCORD_WEBHOOK_CONVENTION_PREP,
  `📦 **Convention in ${daysUntil} day(s): ${eventName}**
` +
  `Low stock items:
${lowStockList}`
);
...
**#print-watch** — fires on significant print milestones: print started, print completed, layer count updates on long prints.
```javascript
// In server.js Pi hub — trigger on MQTT gcode_state changes
// Print started
if (gcodeState === 'RUNNING' && prevState !== 'RUNNING') {
  await postToDiscord(
    process.env.DISCORD_WEBHOOK_PRINT_WATCH,
    `🖨️ **Print started** on ${printerName}
File: ${subtaskName}`
  );
}
// Print completed
...
All 3 use the same `postToDiscord()` helper already defined above in Section 21. Env vars are already set on the Pi.
